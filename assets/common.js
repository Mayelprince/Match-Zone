...
//  Dev:  xxxxxxx
//  Prod: https://api.princetechn.com
// ════════════════════════════════════════════════
...
const matchCache = {};
let _mkc = 0;
function cacheMatch(m) { const k = 'mk' + (_mkc++); matchCache[k] = m; return k; }
function cacheStream(s) { const k = 'sk' + (_mkc++); matchCache[k] = s; return k; }
function clearCache() { Object.keys(matchCache).forEach(k => delete matchCache[k]); _mkc = 0; }

// ── NEWS CACHE ──
const newsCache = {};
let _nkc = 0;
function cacheNews(n) { const k = 'nk' + (_nkc++); newsCache[k] = n; return k; }

// ── HELPERS ──
function sportChip(sport) {
  const s = (sport || '').toLowerCase();
  const label = s === 'football' ? 'Football' : s === 'basketball' ? 'Basketball' : s === 'tennis' ? 'Tennis' : (sport || 'Sport');
  return `<span class="sport-chip ${s || 'other'}">${label}</span>`;
}

function statusClass(status) {
  const s = (status || '').toLowerCase();
  if (s.includes('live') || s.includes('1st') || s.includes('2nd') || (s.includes('half') && !s.includes('half time'))) return 'live';
  if (s.includes('full') || s.includes('ft') || s.includes('ended') || s.includes('finished')) return 'ft';
...
function isFinished(status) {
  const s = (status || '').toLowerCase();
  return s.includes('full') || s.includes('ft') || s.includes('ended') || s.includes('finished');
}
...
function nfpToggleControls() {
  if (_nfpVisible) { nfpHideControls(); } else { nfpShowControls(); }
}

function openStreamByKey(sk) {
  const s = matchCache[sk];
  if (!s || !s.url) return;
...
function openStreamForMatch(matchKey, streamKey) {
  ...
  openStreamByKey(streamKey);
}

function playStreamIfLive(matchKey, url, title) {
  const m = matchCache[matchKey];
  if (m && isFinished(m.status)) { showMatchEndedScreen(m); return; }
...
function playStream(url, title) {
  ...
  if (!player) { window.open(url, '_blank', 'noopener'); return; }
  ...
  if (isHLS && video) {
    ...
    if (qualBadge) { qualBadge.textContent = 'Connecting…'; qualBadge.classList.add('show'); }
    ...
    const _retryViaProxy = () => {
      ...
      const proxyUrl = '/matchzone/data/stream-proxy?url=' + encodeURIComponent(url);
      if (qualBadge) { qualBadge.textContent = 'Retrying…'; qualBadge.classList.add('show'); }
    ...
    if (window.Hls && Hls.isSupported()) {
      ...
      const hls = new Hls({ maxBufferLength: 30, enableWorker: true, fragLoadingMaxRetry: 1, levelLoadingMaxRetry: 1, manifestLoadingMaxRetry: 1 });
      window._hlsInst = hls;
      hls.loadSource(url);
      ...
      hls.on(Hls.Events.MANIFEST_PARSED, () => {
        video.play().catch(() => {});
        if (qualBadge) { qualBadge.textContent = 'HLS'; qualBadge.classList.add('show'); }
        if (!url.includes('stream-proxy?')) {
      ...
      if (!url.includes('stream-proxy?')) {
        ...
        video.addEventListener('timeupdate', function _c() {
          clearTimeout(window._stallTimer); window._stallTimer = null;
...
function toggleMute() {
  ...
  if (v.muted) {
    ...
    if (icon) icon.className = vol > 0.5 ? 'fas fa-volume-up' : vol > 0 ? 'fas fa-volume-down' : 'fas fa-volume-mute';
...
function setVolume(val) {
  ...
  v.volume = f; v.muted = f === 0;
  if (icon) icon.className = f > 0.5 ? 'fas fa-volume-up' : f > 0 ? 'fas fa-volume-down' : 'fas fa-volume-mute';
}
...
async function togglePiP() {
  ...
  const setPipActive = (on) => pipBtns.forEach(b => b.classList.toggle('pip-active', on));
  try {
    if (document.pictureInPictureElement) {
      ...
      v.addEventListener('loadedmetadata', async () => {
        try { await v.requestPictureInPicture(); setPipActive(true); } catch {}
      }, { once: true });
    }
...
function openMatchModal(key) {
  ...
  document.getElementById('modal-title').textContent = `${m.homeTeam || 'Home'} vs ${m.awayTeam || 'Away'}`;

  const streamsHtml = m.streams && m.streams.length
    ? `<div class="streams-section">
        <div class="streams-section-title">Available Streams</div>
        <div class="streams-list">${m.streams.map(s => {
          ...
          return `<div class="stream-item">
            ...
            <button class="watch-btn" onclick="closeModal();playStreamIfLive('${key}','${safeUrl}','${safeTitle}')"><i class="fas fa-play"></i> Watch</button>
          </div>`;
        }).join('')}
        </div>
       </div>` : '';
  ...
  const infoRows = [
    m.league ? `<div class="info-item"><div class="info-label">League</div><div class="info-value">${m.league}</div></div>` : '',
    m.sport ? `<div class="info-item"><div class="info-label">Sport</div><div class="info-value" style="text-transform:capitalize">${m.sport}</div></div>` : '',
    m.date ? `<div class="info-item"><div class="info-label">Date</div><div class="info-value">${m.date}</div></div>` : '',
    m.time ? `<div class="info-item"><div class="info-label">Time</div><div class="info-value">${m.time}</div></div>` : '',
    m.startTime ? `<div class="info-item"><div class="info-label">Kick-off</div><div class="info-value">${formatDate(m.startTime)}</div></div>` : '',
    m.halfTimeScore ? `<div class="info-item"><div class="info-label">HT Score</div><div class="info-value">${m.halfTimeScore}</div></div>` : '',
  ].filter(Boolean).join('');
...
function openNewsModal(key) {
  ...
  const thumb = (n.cover?.url) || (typeof n.cover === 'string' ? n.cover : '') || n.thumbnail || n.image || '';
  ...
  const dateStr = ts ? new Date(Number(ts) || ts).toLocaleDateString([], { weekday: 'long', month: 'long', day: 'numeric', year: 'numeric' }) : '';
  const catVal = n.category;
  const category = typeof catVal === 'string' ? catVal : (typeof catVal === 'number' ? 'Sports' : catVal?.name || 'Sports');
  ...
  document.getElementById('news-modal-title').textContent = title.length > 50 ? title.substring(0, 50) + '…' : title;
  bg.classList.add('open');
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (mbg) mbg.addEventListener('click', e => { if (e.target === mbg) closeModal(); });
  ...
  if (nfpEl) {
    ...
    nfpEl.addEventListener('touchstart', () => {
  ...
  if (videoEl) {
    videoEl.addEventListener('play', () => {
      ...
      if (bi) bi.className = 'fas fa-pause';
      const bigBtn = document.querySelector('.nfp-big-btn');
      if (bigBtn) setTimeout(() => { if (!videoEl.paused) bigBtn.classList.add('nfp-hidden'); }, 800);
  ...
  document.addEventListener('leavepictureinpicture', () => {
    document.querySelectorAll('.nfp-pip-btn').forEach(b => b.classList.remove('pip-active'));
  });
